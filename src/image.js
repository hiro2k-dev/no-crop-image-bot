...
async function noCropBuffer(buf, ratio, borderHex, inputFormatHint) {
  ...
  if (Ch < H) { Ch = H; Cw = Math.ceil(r * Ch); }
  ...
  let pipeline = base.extend({ top, bottom, left, right, background: borderHex });

  if (fmt === 'jpeg') pipeline = pipeline.jpeg({ quality: 100, chromaSubsampling: '4:4:4' });
  else if (fmt === 'png') pipeline = pipeline.png({ compressionLevel: 9 });
  else if (fmt === 'webp') pipeline = pipeline.webp({ quality: 100, lossless: true });
  else if (fmt === 'tiff') pipeline = pipeline.tiff({ quality: 100 });
  else if (fmt === 'avif') pipeline = pipeline.avif({ quality: 100, lossless: true });
  else if (fmt === 'heif') pipeline = pipeline.heif({ quality: 100, lossless: true });
